...
export default function AnalyzeButton({ 
  ...
  return (
    <button
      type={type}
      onClick={onClick}
      className={`relative inline-flex items-center justify-center px-8 py-2.5 overflow-hidden tracking-tighter text-white bg-gray-800 rounded-md group cursor-pointer ${className}`.trim()}
    >
      <span
      ...
      ></span>
      <span className="relative text-sm font-semibold flex items-center gap-2">
        {children}
      </span>
    </button>
  );
